...
public class SQLitePersistence extends AbstractJDBCPersistence {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SQLitePersistence.class);
    ...
    protected Connection createConnection() throws SQLException, IOException {
        try {
            ...
            throw new SQLException("Could not find the SQLite driver.", e);
        }
        
        File f = resolveFile();
        LOG.info("Using database file at: "+f);
        Connection conn = DriverManager.getConnection("jdbc:sqlite:".concat(f
                .getAbsolutePath()));
