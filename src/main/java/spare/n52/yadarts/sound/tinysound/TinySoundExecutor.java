...
public class TinySoundExecutor implements SoundExecutor {

	public static final Logger logger = LoggerFactory.getLogger(TinySoundExecutor.class);
	private Map<SoundId, Sound> idToSoundMap = new HashMap<>();
	public boolean running = true;
	private ExecutorThread executor;
        private String soundPackageName;
	
	public TinySoundExecutor() {
	}
	...
	private SoundSequence resolveSounds(List<SoundId> theSounds) {
		...
		for (SoundId sound : theSounds) {
			if (!idToSoundMap.containsKey(sound)) {
				idToSoundMap.put(sound, new Sound(soundPackageName, sound));
			}
